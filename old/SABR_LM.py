...
import old.sigma_sabr as sigma_sabr
reload(sigma_sabr)
from old.sigma_sabr import Levenberg_Marquardt, get_residals, proj_sabr

class SABR:
    def __init__(self, data: pd.DataFrame, K, F, iv0, weights, r, beta: float = 0.5,
    ...
    def run(self) -> None:
        ...
        Niter = 50
        sabr_params = np.array([1, 10, 0.5, 0.0])
        optimum_scipy = Levenberg_Marquardt(Niter, get_residals, proj_sabr, sabr_params,
        KK = self.K, FF = self.F, iv00 = self.iv0, weightss= self.weights, rr = self.r
        )
        # optimum_scipy = self._minimize_scipy()
        self.alpha_scipy, self.v_scipy, self.beta, self.rho_scipy,  = optimum_scipy["x"]
        logger.info(
